...
class EncoderOdom: 
	...
	self.odom_pub1 = rospy.Publisher('/odom1', Polar, queue_size=10)
        self.cur_x = 0
        self.cur_y = 0
        self.cur_theta = 0
	...
	#rospy.Subscriber("/encoders", Polar, callback1)
    ...
    def update(self, enc_left, enc_right):
        ...
        dist = (dist_right + dist_left) / 2.0   #parcourue par periode
	...
	p.X=self.cur_x
	p.Y=self.cur_y
	p.Theta = self.cur_theta
	print(p)
	self.odom_pub1.publish(p)
    ...
    def update_publish(self, enc_left, enc_right):
        ...
        if abs(enc_left - self.last_enc_left) > 20000:
            ...
            self.update(enc_left, enc_right)
...
def my_callback(data):
	global status1, status2, enc1, enc2, crc1, crc2
...
class Node:
    def __init__(self):
	global status1, status2, enc1, enc2, crc1, crc2
	status1, status2, enc1, enc2, crc1, crc2 = 0, 0, 0, 0, 0, 0
	...
	rospy.Subscriber("/encoders", Encoders, my_callback)
        ...
        self.TICKS_PER_METER = float(rospy.get_param("~tick_per_meter", "16300"))
        self.BASE_WIDTH = float(rospy.get_param("~base_width", "0.200"))
	...
	global status1, status2, enc1, enc2, crc1, crc2
        ...
        while not rospy.is_shutdown():
            ...
            #status1, enc1, crc1 = 0, 0, 0
            #status2, enc2, crc2 = 0, 0, 0

            '''try:
		#if roboclaw.ReadBuffers(self.address)[1] is 128:
